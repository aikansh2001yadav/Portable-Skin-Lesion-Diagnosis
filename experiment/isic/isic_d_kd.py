...
from src.utils import create_label_encoder, load_trained_model, print_cli_arguments, print_error_message, \
    print_num_gpus_available, print_runtime, set_random_seeds

from src.cnn_models.cnn_model import CnnModel, test_model_evaluation
...
def main (_folder, _csv_path_train, _imgs_folder_train, _csv_path_test, _imgs_folder_test, _lr_init, _sched_factor,
    ...
    imagesT, labelsT = import_cbisddsm_testing_dataset(l_e)
    test_dataset = create_dataset(imagesT, labelsT)
    ...
    test_model_kd (models, test_dataset, checkpoint_path=_checkpoint_best, feat_fn=feat_fn, loss_fn=loss_fn, save_pred=True,
                partition_name='test', metrics_to_comp='all', class_names=labelsT, metrics_options=_metric_options,
